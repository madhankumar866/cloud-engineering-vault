...
- **Relates to:** [[Terraform/1. HCL Fundamentals|1. HCL Fundamentals]] — `for_each` and `count` are meta-arguments on resource blocks
- **Relates to:** [[Terraform/3. Modules|3. Modules]] — `for_each` on a `module {}` block creates multiple module instances
