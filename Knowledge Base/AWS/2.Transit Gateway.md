...
The key is that TGW route tables separate two different responsibilities:

- **Association** decides which route table an attachment uses for outbound route lookup.
  - Example: `Prod VPC` is associated with the Prod route table, `Dev VPC` is associated with the Dev route table.
- **Propagation** decides which route table(s) learn an attachment's CIDR.
  - Example: `Shared Services` propagates into both Prod and Dev route tables so both can reach it.

Without association, the TGW would not know which routing map an attachment should use. Without propagation, a route table would not have the advertised route for another attachment, so traffic could not flow.

That means the two controls work together:

- Association = “I am a Prod attachment, so use the Prod routing map.”
- Propagation = “Advertise my network into these route tables so they can route to me.”
...
#flashcards/aws/10_transit_Gateway
...
**Can a single TGW attachment be associated with multiple TGW Route Tables simultaneously, or propagate into multiple TGW Route Tables simultaneously?**
?
An attachment can be **associated with EXACTLY ONE** route table at a time (for outbound route lookup), but it can **propagate its CIDR into MULTIPLE** route tables simultaneously (so different segments can route back to it, like in Shared Services setups).

---

**In a multi-account organization using AWS RAM to share a central Transit Gateway, which account is responsible for creating the TGW Attachment?**
?
The **Spoke Account (VPC owner)** must create the TGW Attachment in their VPC and update their local subnet route tables to target `tgw-xxxxxxxx`. The central networking Hub Account only shares the TGW resource via RAM.

---

**How many total Transit Gateway attachments are required to interconnect $N$ VPCs in a hub-and-spoke topology?**
?
Exactly $N$ attachments (one per VPC), avoiding the complex full-mesh scaling equation ($N \times (N - 1) / 2$) required by non-transitive VPC Peering.

---

**Does automatic route propagation work across inter-region TGW Peering connections?**
?
No. Across region-to-region TGW Peering connections, automatic propagation is not supported. You must manually configure static routes in each regional TGW Route Table.
